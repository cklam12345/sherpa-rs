...
fn main() -> Result<()> {
    ...
    let (sample_rate, mut samples) = read_audio_file(test_file)?;
    ...
    while index + window_size <= samples.len() {
        ...
        if vad.is_speech() {
            while !vad.is_empty() {
                ...
                if embedding_manager.search(&embedding, 0.4).is_some() {
                    println!("Voice authenticated successfully!");
                } else {
                    println!("Voice authentication failed.");
                }
                vad.pop();
            }
        }
